...
void timer_start(GameScreen screen, Timer *t) {
    if (screen == GAME_SCREEN_PLAYING) {
        t->last_time = 0.0;
...
void timer_report(Timer *t) {
    ...
    if (t->timed_module < 0 || t->timed_module > 3) {
        return;
    }

    //Obtiene el numero de fila y columna desde los cuales se desea empezar a imprimir la información
    int row = 3 + (t->timed_module * 4);
    int col = GAME_WIDTH + 10;

    //Obtiene el nombre el módulo que ha sido monitoreado
    const char *module_name;
    switch(t->timed_module) {
        case 0:
            module_name = "render.c";
            break;
        case 1:
            module_name = "collision.c";
            break;
        case 2:
            module_name = "projectile.c";
            break;
        case 3:
            module_name = "Tiempo Total";
            break;
    }
...
void timers_render(Timer *t1, Timer *t2, Timer *t3, Timer *t4) {
    timer_report(t1);
    timer_report(t2);
    timer_report(t3);
    timer_report(t4);
